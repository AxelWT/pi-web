...
test("allows same-hostname requests despite scheme/port divergence behind a proxy", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  // Browser-facing site is https://example.internal, but the reverse proxy
  // (e.g. an HTTP vhost plugin without X-Forwarded-Proto) hands the backend a
  // bare http://... request.url. Only the Host header is preserved.
  ...
  assert.equal(isApiRequestAllowed(request, ["example.internal"]), true);
});
