...
// Per-tab secret generated by lib/wetty-manager.ts for each spawn. Every
// socket.io client must present this in `handshake.auth.token` before any
// pty is spawned. This is defense-in-depth: pty-server still binds to
// 127.0.0.1, so the token only matters if someone misconfigures PTY_HOST
// to 0.0.0.0 (which would otherwise expose an unauthenticated shell).
...
// Reject any connection that does not present the per-tab auth token. The
// token is generated by lib/wetty-manager.ts and passed in via PTY_AUTH_TOKEN
// when this subprocess is spawned. Both the browser (direct mode) and the
// same-origin proxy (proxy mode) must send it as `auth.token` in the
// socket.io handshake. Without this check, binding PTY_HOST to anything
// other than 127.0.0.1 would expose an unauthenticated shell to the network.
io.use((socket, next) => {
  if (!AUTH_TOKEN) {
    return next(new Error("pty-server misconfigured: no auth token"));
  }
